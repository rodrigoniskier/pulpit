// Seletor restaurado a partir da estrutura do projeto original GPLv3 de Genilson Felinto.
...
function fillSelect(select,values,placeholder='Selecione…'){
  ...
  for(const value of values){ const o=document.createElement('option'); o.value=String(value); o.textContent=String(value); select.append(o); }
  select.disabled=false;
}
function range(n){ return Array.from({length:n},(_,i)=>i+1); }

export class BibleSelector {
  constructor(){ this.target=null; }
  init(){
    const modal=$('#bible-selector-modal');
    if(!modal) return;
    fillSelect($('#picker-book'),Object.keys(BIBLE_STRUCTURE),'Selecione um livro…');
    document.addEventListener('click',e=>{
      const input=e.target.closest?.('.passage-picker');
      if(input){ e.preventDefault(); this.open(input); }
      const button=e.target.closest?.('[data-passage-target]');
      if(button){ const target=document.querySelector(button.dataset.passageTarget); if(target)this.open(target); }
    });
    $('#picker-book').addEventListener('change',()=>this.bookChanged());
    $('#picker-chapter-start').addEventListener('change',()=>this.chapterStartChanged());
    $('#picker-verse-start').addEventListener('change',()=>this.startVerseChanged());
    $('#picker-chapter-end').addEventListener('change',()=>this.chapterEndChanged());
    $('#picker-verse-end').addEventListener('change',()=>this.preview());
    $('#picker-cancel').addEventListener('click',()=>modal.close());
    $('#picker-cancel-secondary')?.addEventListener('click',()=>modal.close());
    $('#picker-apply').addEventListener('click',()=>this.apply());
    modal.addEventListener('close',()=>{this.target=null;});
  }
  open(input){
    this.target=input;
    $('#picker-book').value='';
    for(const id of ['#picker-chapter-start','#picker-verse-start','#picker-chapter-end','#picker-verse-end']){
      const el=$(id); el.replaceChildren(); el.disabled=true;
    }
    $('#picker-preview').textContent='—';
    $('#picker-apply').disabled=true;
    $('#bible-selector-modal').showModal();
  }
  bookChanged(){
    const book=$('#picker-book').value;
    if(!book) return;
    fillSelect($('#picker-chapter-start'),range(BIBLE_STRUCTURE[book].length),'Capítulo…');
    $('#picker-preview').textContent=book;
  }
  chapterStartChanged(){
    const book=$('#picker-book').value, chapter=Number($('#picker-chapter-start').value);
    if(!book||!chapter)return;
    fillSelect($('#picker-verse-start'),range(BIBLE_STRUCTURE[book][chapter-1]),'Versículo…');
    fillSelect($('#picker-chapter-end'),range(BIBLE_STRUCTURE[book].length).filter(n=>n>=chapter),'Mesmo capítulo');
    $('#picker-chapter-end').value=String(chapter);
    this.chapterEndChanged();
  }
  startVerseChanged(){ this.chapterEndChanged(); }
  chapterEndChanged(){
    const book=$('#picker-book').value, startChapter=Number($('#picker-chapter-start').value), endChapter=Number($('#picker-chapter-end').value||startChapter);
    if(!book||!startChapter)return;
    fillSelect($('#picker-verse-end'),range(BIBLE_STRUCTURE[book][endChapter-1]),'Versículo final…');
    const startVerse=Number($('#picker-verse-start').value);
    if(endChapter===startChapter&&startVerse){
      [...$('#picker-verse-end').options].forEach(o=>{if(o.value&&Number(o.value)<startVerse)o.disabled=true;});
      $('#picker-verse-end').value=String(startVerse);
    }
    this.preview();
  }
  preview(){
    const book=$('#picker-book').value, cs=Number($('#picker-chapter-start').value), vs=Number($('#picker-verse-start').value);
    if(!book||!cs){ $('#picker-preview').textContent='—'; $('#picker-apply').disabled=true; return; }
    const ce=Number($('#picker-chapter-end').value||cs), ve=Number($('#picker-verse-end').value||vs);
    let text=book==='Salmos'?`${book} ${cs}`:`${book} ${cs}`;
    if(vs){
      text+=`:${vs}`;
      if(ve&&(ce!==cs||ve!==vs)) text+=ce!==cs?`-${ce}:${ve}`:`-${ve}`;
    }else if(ce!==cs){
      text+=`-${ce}`;
    }
    $('#picker-preview').textContent=text;
    $('#picker-apply').disabled=false;
  }
  apply(){
    if(!this.target)return;
    const value=$('#picker-preview').textContent;
    if(!value||value==='—')return;
    this.target.value=value;
    this.target.dispatchEvent(new Event('input',{bubbles:true}));
    this.target.dispatchEvent(new Event('change',{bubbles:true}));
    $('#bible-selector-modal').close();
  }
}
